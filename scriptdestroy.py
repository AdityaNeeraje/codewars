...
def ActPirate(pirate):
    ...
    if (
        ...
        if pirate.getTeamSignal() == "":
            ...
            team_assigned = pirate.getTeamSignal()[-1]
            team_to_be_assigned = "1" if team_assigned == "0" else "0"
            sig=right[-1] + "," + str(x + 1) + "," + str(y) + "," + team_to_be_assigned
            s=pirate.getTeamSignal()
            pirate.setTeamSignal(s+" "+sig)
    if n % 2 == 0 and n <= dimY:
        if (pirate.getSignal() != "1") and ((pirate.getPosition()[0] != dimX-1 and x_start == 0) or (pirate.getPosition()[0] != 0 and x_start == dimX-1)):
            if (pirate.getPosition()[0] == dimX-2 and x_start == 0) or (pirate.getPosition()[0] == 1 and x_start == dimX-1):
                pirate.setSignal("1")
            return Sweep(x_start, y_start, pirate, dimX, dimY, n)
        else:
            if (pirate.getPosition()[0] == 1 and x_start == 0) or (pirate.getPosition()[0] == dimX-2 and x_start == dimX-1):
                pirate.setSignal("")
            return Attack(x_start, y_start, pirate, dimX, dimY, n, x_end, y_end)
    if n % 2 == 1 and n <= dimX:
        if (pirate.getSignal() != "1") and ((pirate.getPosition()[1] != dimY-1 and y_start == 0) or (pirate.getPosition()[1] != 0 and y_start == dimY-1)):
            if (pirate.getPosition()[1] == dimY-2 and y_start == 0) or (pirate.getPosition()[1] == 1 and y_start == dimY-1):
                pirate.setSignal("1")
            return Sweep(x_start, y_start, pirate, dimX, dimY, n)
        else:
            if (pirate.getPosition()[1] == 1 and y_start == 0) or (pirate.getPosition()[1] == dimY-2 and y_start == dimY-1):
                pirate.setSignal("")
            return Attack(x_start, y_start, pirate, dimX, dimY, n, x_end, y_end)
    if pirate.getTeamSignal() != "":
        s = pirate.getTeamSignal()
